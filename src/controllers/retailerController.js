...
const getRetailer = async (req, res) => {
  ...
  await pool.query(
    "SELECT * FROM rb.retailer_setting ORDER BY retailer_name ASC",
    (error, results) => {
      if (error) {
        throw error;
      }
...
const RetailerState = async (req, res) => {
  ...
  await pool.query(
    "SELECT distinct retailer_state FROM rb.retailer_setting",
    (error, results) => {
      if (error) {
        throw error;
      }
  ...
  await pool.query(
    ...
    (error, results) => {
      ...
      if (!results.rows.length) {
        pool.query(
          "SELECT * FROM rb.retailer_setting WHERE retailer_name=$1",
          [retailer_name],
          (error, results) => {
            if (error) {
              throw error;
